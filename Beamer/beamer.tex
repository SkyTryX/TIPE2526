...
\begin{frame}{La logique du 1er ordre}
    \begin{itemize}
        ...
        \item Un ensemble de fonctions de symboles $\mathcal{S}_F$ qui a des symboles associe un symbole
        \item Un ensemble de symbole relationnel $\mathcal{S}_p$ qui a des symboles associe un booléen.
    \end{itemize}
    On note l'arité le nombre d'argument d'une fonction.
    \begin{definition}
        On définit les termes $\mathcal{T}$ récursivement:
        \begin{itemize}
            \item $\mathcal{T}_0 = \mathcal{V}\cup\mathcal{S}_F^0$ qui dénote les fonctions d'arité $0$.
            \item $\mathcal{T}_n = \mathcal{T}_{n-1}\cup\{ f(t_1,\dots, t_k) | k\in\mathbb{N} \land f\in\mathcal{S}_F^k \land t_1,\dots,t_k\in\mathcal{T}_{n-1} \}$ 
            \item $\mathcal{T} = \bigcup_{n\in\mathbb{N}} \mathcal{T}_n$
        \end{itemize}
    \end{definition}
\end{frame}

\begin{frame}{La méthode des tableaux pour logique du 1er ordre}
    On ajoute deux règles: une sur les $\forall$ et une pour les $\exists$:
    ...
    Pour la 1ère règle, on peut prendre $t$ différent, pour la deuxième, on ne peut pas!
    En effet, $c$ est une constante fraiche qu'on construit par skolémisation.

    A Corriger
\end{frame}

\begin{frame}{Une étude en premier ordre}
    Je ne sais pas si je ne devrais pas plutot commencer par une étude dans le 1er ordre, ce serait + smart
\end{frame}

\begin{frame}{Définition - Logique Modale}
    \begin{definition}[Logique Modale]
        C'est une logique qui étend la logique propositionnelle, la logique du premier ordre avec des modalités
    \end{definition}
    \pause
    \begin{definition}[Modalite]
        Terme modifiant le rapport entre un prédicat et un sujet.
    \end{definition}
    \pause
    Il existe plusieurs types de modalité:
    \begin{itemize}
        \item Il est nécessaire que ... $\square$
        \item modalité temporelle: “Demain, ...” $X$
        \item modalité épistémique: “Alice croit que Paul sait que ... ”($B_{Alice}$, $K_{Paul}$ ...)
        \item modalité déontique: “Il est obligatoire ...” $O$, “Il est permis ...” $P$
        \item Ou: $p \implies[\pi]q$: Si $p$ avant l'exécution de $\pi$, alors $q$ après”
    \end{itemize}
\end{frame}
