...
\title{Méthode des tableaux: Application en logique propositionelle et en logique linéaire temporelle}
...
\begin{frame}{Intro - Positionnement}
    \begin{itemize}[<+->]
        \item On souhaite étudier une méthode algorithmique permettant de montrer la satisfiabilité d'une formule: la Méthode des tableaux.
        \item Elle consiste à construire un arbre avec la formule à la racine, et à utiliser des règles pour développer et créer des branches.
...
\begin{frame}[fragile]{Intro - Exemple}
\centering
    \begin{tikzpicture}[
    level distance=15mm,
    ...
    box/.style={draw, rounded corners, inner sep=3pt, align=left, text width=48mm}
    ]
    \node[box] (root) {1. $\lnot\bigl(a\Rightarrow(b\Rightarrow a)\bigr)$}
        child { node[box] (n) {2. $a$, $\lnot(b\Rightarrow a)$  (Règle $\lnot\Rightarrow$)}
        child { node[box] (m) {3. $b$, $\lnot a$ (Règle $\lnot\Rightarrow$)}
        }
    };
    \draw[->, red, thick] (m.south) .. controls +(8mm,-8mm) and +(8mm,8mm) .. (n.north);
    \end{tikzpicture}
    
    Contradiction: $a$ (2) et $\lnot a$ (3). Conclusion, cette formule n'est pas satisfiable.
\end{frame}

\begin{frame}{LP - Définition de l'étude}
...
\begin{frame}{LP - reécriture}
    On re-écrit l'arbre induit par la méthode des tableaux d'une manière différente (ici pour $\varphi = a_1\land(a_2\lor(a_3\land(a_4\lor a_5)))$):
...
\begin{frame}{LP - Preuves et Analyse}
    ...
    \begin{itemize}
        ...
        \item \textbf{Méthode des tableaux:} \dots
        \item \textbf{Quine (avec conversion en CNF):} 0.025874s
        \item \textbf{Quine (sans conversion en CNF):} 0.018691s
    \end{itemize} 
\end{frame}
 
\begin{frame}{LP - Conclusion}
    \textbf{Remarques:}
    \begin{itemize}
        \item Bien entendu, il n'est pas possible de convertir toutes les formules en Alternée.
        \item On a donc trouvé un exemple de type de formule qui est facile à analyser par méthode des tableaux.
    \end{itemize}

    Nous allons maintenant ellargir notre étude et s'interesser à la logique linéaire temporelle (LTL).
\end{frame}

\begin{frame}{LTL - Intro 1}
...
\begin{frame}{LTL - Intro 2}
    ...
    \begin{definition}[Satisfaction d'un monde ($\omega \models f$)]
        \begin{itemize}
...
\begin{frame}{LTL - Verification de Modèle}
    ...
    CF par Exemple: A. Pnueli and L. Zuck. Probabilistic verification by tableaux. In 1st Annual Symposium on Logic in Computer Science (LICS), pages 322-331. IEEE Computer Society Press, 1986

    Ainsi, nous allons étudier un exemple de formule de la LTL et étudier sa résolution par méthode des tableaux.
\end{frame}

\begin{frame}{LTL - Implementation utilisée}
    En réalité, la version de la méthode des tableaux que je présente en LTL est celle de Mark Reynolds, qui est une approche "tree-style"
    plus simple que les autres implémentations, mais qui reste correct.
    Faire une bonne implémentation d'une telle méthode reste assez fastidieux et il n'est pas astucieux ici de créer nous même
    une implémentation. C'est pour ça que l'on va s'aider de l'implémentation de Anatole Leterrier trouvable ici:
    \begin{center}
        https://github.com/anatcaramba/LTL-SAT-Solver-by-Reynolds-Tableaux
    \end{center}

    Nous allons maintenant étudier une application de la méthode des tableaux.
\end{frame}

\begin{frame}{LTL - Méthode des tableaux 1}
    On rajoute donc les règles suivantes:
    ...
    En plus on rajoute des règles supplémentaires:
    \begin{definition}
        On rappelle \textbf{X-eventualite} les formules de la forme $\textbf{X}(a\textbf{U}b)$ et $\textbf{XF}b$   
    \end{definition}
    On notera $x\leq y$ si $y$ est descendant de $x$ (resp < pour descendant strict)
\end{frame}

\begin{frame}{LTL - Méthode des tableaux 2}
    \begin{itemize}
...
\begin{frame}{LTL- Introduction de l'Etude}
    \textbf{Remarques:}
    \begin{itemize}
        \item Grâce à toutes ces règles, on peut montrer qu'un tel algorithme de résolution de SAT est complet et correct (il renvoie vrai si la formule est satisfiable et reciproquement).
        \item On sait que typiquement, les propriétés que doivent satisfaire notre modèle est modélisé par des formules logiques en LTL.
    \end{itemize}

    Ainsi nous allons nous interesser à la modélisation d'un objet technologique en LTL, en observant les hypothèses, que l'on fait etc... 
    
    In fine, on pourra appliquer la méthode des tableaux pour conclure sur le bon fonctionnement de tout!
\end{frame}
